...
class MatrixAssembler2D():
    def __init__(self, assembly_interface: AssemblyInterface2D, grid_domain: GridDomain):
        ...
        self.I_lr = np.array([[0.62584577, 0.74287624, -0.69579697, 0.32707497],
        ...
        self.I_rl = np.array([[0.62584577, 0.74287624, -0.69579697, 0.32707497],
                              [0.37035602, 0.71462439, -0.10993613, 0.02495572],
                              [0.05668829, 0.63887521, 0.44998043, -0.14554393],
                              [-0.13696959, 0.43779799, 0.81535681, -0.11618522],
                              [-0.10971663, 0.36700592, 0.84584219, -0.10313148],
    ...
    def merge_two_cells(self, cell_1, cell_2):
        ...
        dist_1 = distributed_eye(pairtuples=pairtuple_1,
                                 shape=(len(pairtuple_1), self.assembly_interface.get_ddof_count()))
        dist_2 = distributed_eye(pairtuples=pairtuple_2,
                                 shape=(len(pairtuple_2), self.assembly_interface.get_ddof_count()))

        glob += dist_1.T * self._get_local_gg_matrix(distribution=props_1[1], order=props_1[0]) * dist_1
        glob += dist_2.T * self._get_local_gg_matrix(distribution=props_2[1], order=props_2[0]) * dist_2
        return glob
    ...
    def _distribute_one_cell(self, cell: Cell2D):
        ...
        dist_1 = distributed_eye_easy(pairtuples=pairtuple, axis2shape=self.assembly_interface.get_ddof_count())
        glob += dist_1.T * self._get_local_gg_matrix(distribution=props[1], order=props[0]) * dist_1
        return glob
    ...
    def assemble(self):
        ...
        for num, props in enumerate(self.assembly_interface.iterate_ddofs_and_wconn()):
            ...
            for (k_edge, adj_cells), (w_edge, weak_conns) in zip(props['adj_cells'].items(), props['wconn'].items()):
                ...
                print(tmp_merged[self.stack_wcon_dofs(weak_conns)])
